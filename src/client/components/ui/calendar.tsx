...
const CalendarHead: Component<ComponentProps<"div">> = (props) => {
  ...
  return (
    <div
      class={cn(
        "relative flex w-full items-center justify-between pt-1 pb-4",
        local.class,
      )}
      {...others}
    />
...
type CalendarTriggerProps<T extends ValidComponent = "button"> =
  & Omit<NavProps<T>, "action">
  & {
...
const CalendarPrevTrigger = <T extends ValidComponent = "button">(
  ...
  const [local, others] = splitProps(props as CalendarTriggerProps, [
    "class",
    "children",
  ]);
  return (
...
const CalendarGridHead: Component<ComponentProps<"thead">> = (props) => (
  <thead {...props} />
);
...
type CalendarGridHeadCellProps<T extends ValidComponent = "th"> =
  & HeadCellProps<T>
  & {
    class?: string | undefined;
  };
...
const CalendarGridHeadCell = <T extends ValidComponent = "th">(
  ...
  const [local, others] = splitProps(props as CalendarGridHeadCellProps, [
    "class",
  ]);
  return (
    <CalendarPrimitive.HeadCell
      class={cn(
        "w-8 rounded-md font-normal text-muted-foreground text-[0.8rem]",
        local.class,
      )}
      {...others}
    />
...
const CalendarGridBody: Component<ComponentProps<"tbody">> = (props) => (
  <tbody {...props} />
);
...
type CalendarGridBodyCellProps<T extends ValidComponent = "td"> =
  & CellProps<T>
  & {
    class?: string | undefined;
  };
...
const CalendarGridBodyCell = <T extends ValidComponent = "td">(
  ...
  return (
    <CalendarPrimitive.Cell
      class={cn(
        "p-0 has-data-range-end:rounded-r-md has-data-range-start:rounded-l-md has-data-in-range:bg-accent has-[[disabled]]:opacity-40 has-data-in-range:first:rounded-l-md has-data-in-range:last:rounded-r-md",
        local.class,
      )}
      {...others}
    />
...
type CalendarGridBodyCellTriggerProps<T extends ValidComponent = "button"> =
  & CellTriggerProps<T>
  & {
    class?: string | undefined;
  };
