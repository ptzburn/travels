  ...
  return (
    <main
      class={cn(
        ...
        "md:peer-data-[variant=inset]:m-2 md:peer-data-[variant=inset]:ml-0 md:peer-data-[variant=inset]:rounded-xl md:peer-data-[variant=inset]:shadow md:peer-data-[state=collapsed]:peer-data-[variant=inset]:ml-2 peer-data-[variant=inset]:min-h-[calc(100svh-theme(spacing.4))]",
        local.class,
      )}
      {...others}
    />
...
const SidebarMenuSub: Component<ComponentProps<"ul">> = (props) => {
  ...
  return (
    <ul
      ...
      class={cn(
        "mx-3.5 flex min-w-0 flex-col translate-x-px gap-1 border-l border-sidebar-border px-2.5 py-0.5",
        "group-data-[collapsible=icon]:hidden",
        local.class,
      )}
      {...others}
